...
2. **Shell-quoting requirement (never skip this):** requirement/ROADMAP text may include user-authored free text from `/gsd:discuss-phase` and is therefore untrusted with respect to shell metacharacters (`` ` ``, `$(...)`, `"`, `;`, `|`, etc.). Each derived term MUST be passed as its own separate, individually shell-quoted argv element — never concatenate raw extracted text into a single double-quoted Bash string. Prefer invoking the Bash tool with each `--pattern <term>` pair as discrete argv elements (no shell string interpolation of the term's contents); if the tool only accepts a single command string, wrap each term in single quotes and escape any embedded single quote as `'\''`. For each such in-repo file, run via the Bash tool: `gsd-tools context-slice <file> --pattern '<term1>' --pattern '<term2>' ...` BEFORE reading it.
